...
export default function MyOrdersClient() {
  ...
  /**
   * IMPORTANTE:
   * Quitamos orderBy('created_at','desc') para NO necesitar índice compuesto.
   * Luego ordenamos en el cliente por created_at desc.
   */
  ...
  if (!user) {
    return (
      <div className="container mx-auto px-4 py-20 text-center">
        ...
        <Button asChild className="rounded-2xl h-14 px-10 font-bold">
          <Link href="/auth">Ir a Acceso</Link>
        </Button>
      </div>
    );
  }

  // MOSTRAR EL ERROR REAL (para que nunca se quede "silencioso")
  if (error) {
    console.error('MyOrders error:', error);
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
        <AlertCircle className="h-12 w-12 text-destructive" />
        <h3 className="text-xl font-bold font-headline">Error cargando pedidos</h3>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4">
      <div className="mb-10 text-center md:text-left">
        <h1 className="text-4xl font-black font-headline text-slate-900 mb-2">
          Mis Pedidos
        </h1>
                        ...
                        <Badge
                          className={`${
                            order.estado === 'completado'
                              ...
                              ? 'bg-destructive'
                              : 'bg-orange-500'
                          } text-white border-none text-[10px] font-black px-3`}
                        >
                            ...
                            {order.items?.map((item: any, idx: number) => (
                              <div key={idx} className="flex justify-between items-center text-sm">
                                <span className="text-slate-600 font-medium">
                                  {item.nombre}{' '}
                                  <span className="text-slate-400 ml-1">x{item.cant}</span>
                                </span>
